    ...
    function coreNestedPointHtml(point){return '<div class="core-nested-point"><span class="core-nested-dot" aria-hidden="true"></span><div class="core-nested-copy">'+(point.title?'<strong>'+coreEscapeHtml(point.title)+(point.body?'：':'')+'</strong>':'')+(point.body?'<p>'+coreEscapeHtml(point.body)+'</p>':'')+coreQuoteListHtml(point.quotes)+'</div></div>'}
    ...
    function coreTopicHtml(topic,index,media){const feedbackClass=coreIsFeedbackTitle(topic.title)?' feedback-topic':'';return '<section class="core-topic-section'+feedbackClass+'"><header class="core-topic-head"><h4 class="core-topic-title">'+coreEscapeHtml(topic.title)+'</h4>'+(topic.lead?'<p class="core-topic-lead">'+coreEscapeHtml(topic.lead)+'</p>':'')+(topic.paragraphs||[]).map(p=>'<p class="core-topic-para">'+coreEscapeHtml(p)+'</p>').join('')+'</header>'+coreMediaGalleryHtml(media)+(topic.points.length||topic.quotes.length?'<div class="core-topic-body">'+topic.points.map(coreDetailPointHtml).join('')+coreQuoteListHtml(topic.quotes)+'</div>':'')+'</section>'}
    ...
    function coreContentItemHtml(item){const title=coreIntro(item.text)||item.title||'具体内容',open=item.type==='机制'?'':' open';return '<details class="core-content-item"'+open+'><summary class="core-content-summary"><div class="core-content-summary-row">'+coreTagHtml(item.type)+'<h3 class="core-content-title">'+coreEscapeHtml(title)+'</h3><div class="core-content-summary-actions"><span class="core-content-arrow" aria-hidden="true">⌄</span></div></div></summary><div class="core-content-body">'+coreHierarchyHtml(item)+'</div></details>'}
