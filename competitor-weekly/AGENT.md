...
### 4.10 异动卡片文案两个补充坑（20260908 定版）
- **笔误甄别**：人工稿「首页」文案照抄前先肉眼过一遍，明显笔误（如 20260831 期 hp 标题 `9//2`）按上下文格式修正后入库，并在交付说明中显式告知提交人，不静默改、不照抄错误
- **正文长度**：`hero-event-summaries` 卡片正文 **≤50 字**，超出会溢出卡片（20260831 期 df 副标题 75 字被退回截短）；人工稿副标题过长时保留前半句事实、砍掉后置预告句，改动同步告知
