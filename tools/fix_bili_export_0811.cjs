/* 修 bili-rank (3).json：正榜=空title的100条（封面锚点），服务端 view API 补 title/播放量 */
const fs = require('fs');
...
(async () => {
  const src = JSON.parse(fs.readFileSync('C:/Users/wangtingting17/Downloads/bili-rank (3).json', 'utf8'));
  ...
  fs.writeFileSync('G:/创意部/推送看板＆游戏库/twitter-daily-monitor/data/tmp-bili-rank-0811.json', JSON.stringify({ items }, null, 1));
