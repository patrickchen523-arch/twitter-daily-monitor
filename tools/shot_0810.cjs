...
(async () => {
  ...
  await p.waitForTimeout(800);
  await p.screenshot({ path: __dirname + '/shot_0810_top.png' });
  await p.evaluate(() => window.scrollTo(0, 1500));
  await p.waitForTimeout(400);
  await p.screenshot({ path: __dirname + '/shot_0810_boards.png' });
  await p.evaluate(() => window.scrollTo(0, 3200));
  await p.waitForTimeout(400);
  await p.screenshot({ path: __dirname + '/shot_0810_watch.png' });
