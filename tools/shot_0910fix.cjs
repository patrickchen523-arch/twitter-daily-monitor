...
(async () => {
  ...
  await p.evaluate(() => loadLaunchedDate('2026-09-10'));
  await p.waitForTimeout(900);
  await p.screenshot({ path: __dirname + '/shot_0910fix_top.png' });
  await p.evaluate(() => window.scrollTo(0, 1500));
  await p.waitForTimeout(400);
  await p.screenshot({ path: __dirname + '/shot_0910fix_boards.png' });
