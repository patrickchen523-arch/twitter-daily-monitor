/* 0814期：B站导出缺标题/播放量 → view API 服务端批量补全（坑9：带UA+Referer直连，300ms间隔） */
/* 用法: node tools\fix_bili_export_0814.cjs "<b站导出json路径>" → 生成同目录 bili-rank-fixed.json */
const fs = require('fs');
const src = process.argv[2];
if (!src) { console.error('usage: node tools\\fix_bili_export_0814.cjs <bili-rank.json>'); process.exit(1); }
const data = JSON.parse(fs.readFileSync(src, 'utf8'));
const sleep = ms => new Promise(r => setTimeout(r, ms));
(async () => {
  let filled = 0;
  for (const it of data.items) {
    if (it.title && it.state && it.cover && !/^data:/.test(it.cover)) continue;
    try {
