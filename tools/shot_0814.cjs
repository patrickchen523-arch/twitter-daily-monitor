...
(async () => {
  ...
  await p.evaluate(() => loadLaunchedDate('2026-08-14'));
  await p.waitForTimeout(900);
  await p.screenshot({ path: __dirname + '/shot_0814_top.png' });
  await p.evaluate(() => window.scrollTo(0, 1500));
  await p.waitForTimeout(400);
  await p.screenshot({ path: __dirname + '/shot_0814_boards.png' });
  await p.evaluate(() => window.scrollTo(0, 3200));
  await p.waitForTimeout(400);
  await p.screenshot({ path: __dirname + '/shot_0814_watch.png' });
