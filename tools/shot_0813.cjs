...
(async () => {
  ...
  await p.evaluate(() => loadLaunchedDate('2026-08-13'));
  await p.waitForTimeout(900);
  await p.screenshot({ path: __dirname + '/shot_0813_top.png' });
  await p.evaluate(() => window.scrollTo(0, 1500));
  await p.waitForTimeout(400);
  await p.screenshot({ path: __dirname + '/shot_0813_boards.png' });
  await p.evaluate(() => window.scrollTo(0, 3200));
  await p.waitForTimeout(400);
  await p.screenshot({ path: __dirname + '/shot_0813_watch.png' });
